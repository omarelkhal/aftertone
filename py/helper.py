...
def load_text_to_speech(onnx_dir: str, use_gpu: bool = False) -> TextToSpeech:
    ...
    if use_gpu:
        ...
        print(
            "Requesting GPU inference (CUDAExecutionProvider, CPU fallback).\n"
            "  Install: pip install onnxruntime-gpu   (uninstall onnxruntime first if both are present)\n"
            "  Match CUDA/cuDNN to the wheel you install; see onnxruntime GPU docs."
        )
    else:
    ...
    try:
        print(f"Active ONNX Runtime providers: {dp_ort.get_providers()}")
    except Exception:
        pass
